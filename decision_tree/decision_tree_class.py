...
class DecisionTreeRegressor:
    ...
    def _split_node(self, X: np.ndarray, y: np.ndarray, depth: int = 0) -> Node:
        ...
        node = Node(n_samples=X.shape[0], value=np.round(np.mean(y)), mse=self._mse(y))
        if depth == self.max_depth or self.max_depth is None or node.n_samples < self.min_samples_split:
            return node
